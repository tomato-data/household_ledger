...
function Home() {
  ...
  const handleRestoreFromDirectory = (event) => {
    ...
    const backupFiles = files.filter(
      (file) =>
        ...
        file.name.includes("가계부_백업")
    );

    if (backupFiles.length === 0) {
      alert(
        '백업 파일을 찾을 수 없습니다. 파일명이 "가계부_백업"으로 시작하는 JSON 또는 JSON.GZ 파일이 있는지 확인해주세요.'
      );
      return;
    }
  ...
  const restoreFromFile = async (file) => {
    try {
      ...
      // 2. 미리보기로 메타데이터 확인
      const metadata = await previewBackup(token, file);

      // 3. 사용자 확인
      const confirmed = window.confirm(
        `복원할 데이터:\n` +
          `- 카테고리: ${metadata.counts.categories}개\n` +
          `- 트랜잭션: ${metadata.counts.transactions}개\n` +
          `- 반복 트랜잭션: ${metadata.counts.recurring_transactions}개\n\n` +
          `⚠️ 기존 데이터가 모두 삭제되고 백업 데이터로 대체됩니다.\n` +
          `계속하시겠습니까?`
      );
      ...
      // 4. 복원 진행
      await importBackup(token, file, true);

      // 5. Context 새로고침
      await loadCategories();
      await loadAllTransactions();
      await loadRecurringTransactionsd();
      // 6. UI 업데이트
      setShowSidebar(false);
    ...
    <div>
        ...
        <div className="assets-toggle-container">
          ...
          {showAssets && (
            <div className="total-assets-card">
              ...
              <span
                className={`assets-amount ${
                  totalAssets >= 0 ? "positive" : "negative"
                }`}
              >
        ...
        <div className="backup-modal-overlay">
          <div className="backup-modal-content">
            ...
            <div className="backup-info">
              <p>
                💡 일반 백업 기능은 왼쪽 상단 메뉴에서 이용하실 수 있습니다.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="sidebar" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sidebar-content">
              ...
              <div className="sidebar-section">
                ...
                <Link
                  to="/stats"
                  className="sidebar-btn stats-btn"
                  onClick={() => setShowSidebar(false)}
                >
                  📊 통계 보기
                </Link>
      ...
      {showCategoryManagement && (
        ...
        >
          ...
          >
            <CategoryManagement
              onComplete={() => setShowCategoryManagement(false)}
            />
          </div>
        </div>
      )}
