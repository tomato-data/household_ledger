...
> Rails 8 기반 개인 재무 관리 앱 — 별도 인프라 없이, JavaScript 프레임워크 없이 SPA에 가까운 UX를 구현합니다.

## 하이라이트

- **별도 인프라 없음** — SQLite + Solid Trifecta (Queue, Cache, Cable). Redis 없음, npm 없음, DB 서버 없음.
- **SPA 없이 SPA에 가까운 UX** — Hotwire (Turbo + Stimulus)로 실시간 업데이트, 모달, 부분 페이지 교체를 구현.
- **신용카드 할부 자동 분할** — 12개월 할부를 한 번 입력하면 결제일 기준으로 12건의 거래가 자동 생성.
- **베푼 것/받은 것 태깅** — 누구에게 줬는지, 누구에게 받았는지 기록하고 달력에 컬러 도트로 표시. 시중 앱에 없는 기능.
- **한국어 사용에 맞춘 로컬라이징** — 날짜, 통화, UI 전체, Pretendard 폰트까지 한국어 일상 사용에 맞춤.
...
기존 가계부 앱을 여러 개 써봤지만, 원하는 기능을 모두 갖춘 앱은 없었습니다. 신용카드 할부 자동 분할, 베푼 것/받은 것 태깅, 구매일/결제일 기준 통계 전환은 각각 존재했지만, 한 앱 안에 자연스럽게 모여 있지는 않았습니다.

처음에는 회사에서 쓰던 React를 더 익히고 싶어서, AI에게 한 단계씩 물어보며 React + FastAPI + PostgreSQL로 만들었습니다. 이후 Rails 8의 Solid Trifecta(Queue, Cache, Cable)가 Redis 없이 큐, 캐시, WebSocket을 지원한다는 점을 알게 됐고, 개인 프로젝트의 유지보수에는 Rails 모놀리스가 더 잘 맞겠다고 판단해 **Rails 8로 전면 이관**했습니다.
...
| **신용카드** | 카드 등록, 기본 카드 설정, 할부 자동 분할 (UUID 그룹핑), 결제일 기준 날짜 계산, 구매일/결제일 기준 전환 |
...
**이전:** React SPA + FastAPI + PostgreSQL — 서버 3개, CORS 설정, 클라이언트 상태 관리 복잡도.

**이후:** Rails 8 모놀리스 — 단일 프로세스, SQLite 파일 하나, Solid Trifecta (Redis 불필요), Hotwire (JS 프레임워크 불필요).
